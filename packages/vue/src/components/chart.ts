...
const commonProps = {
  ...
  onDrilldown: Function as PropType<(event: ChartPointInteraction | { datum: Datum; index: number; value: number }) => void>,
  onDrilldownBack: Function as PropType<() => void>,
    ...
    setup(props, { attrs, emit }) {
      ...
      const renderLegend = (definitions: readonly ChartSeries<Datum>[], hiddenSeries: readonly string[]) => {
        ...
        return h('div', { 'data-part': 'legend', 'data-placement': config.placement ?? 'bottom', 'data-orientation': config.orientation ?? 'horizontal', style: config.maxHeight ? { maxHeight: `${config.maxHeight}px`, overflowY: 'auto' } : undefined }, [
          config.selectAll !== false && h('button', { type: 'button', 'data-action': 'select-all', onClick: () => { if (props.hiddenSeries === undefined) uncontrolledHiddenSeries.value = []; emit('update:hiddenSeries', []); } }, props.locale?.selectAll ?? defaultChartLocale.selectAll),
          ...definitions.map((item, index) => h('span', { key: item.id }, [h('button', { type: 'button', 'aria-pressed': !hiddenSeries.includes(item.id), onClick: () => { const next = hiddenSeries.includes(item.id) ? hiddenSeries.filter((id) => id !== item.id) : [...hiddenSeries, item.id]; if (props.hiddenSeries === undefined) uncontrolledHiddenSeries.value = next; emit('update:hiddenSeries', next); } }, [h('span', { style: { background: item.color ?? colors[index % colors.length] } }), item.label ?? item.id]), config.isolate && h('button', { type: 'button', 'data-action': 'isolate', 'aria-label': (props.locale?.isolate ?? defaultChartLocale.isolate)(item.label ?? item.id), onClick: () => { const next = definitions.filter((candidate) => candidate.id !== item.id).map((candidate) => candidate.id); if (props.hiddenSeries === undefined) uncontrolledHiddenSeries.value = next; emit('update:hiddenSeries', next); } }, (props.locale?.isolate ?? defaultChartLocale.isolate)(item.label ?? item.id))]))
        ]);
      };
      return () => {
        ...
        const labelPoints = !labelConfig || labelConfig.enabled === false ? [] : flat.reduce<typeof flat>((visible, point) => visible.some((item) => Math.hypot(item.x - point.x, item.y - point.y) < (labelConfig.minDistance ?? 18)) ? visible : [...visible, point], []);
      ...
      const tooltipPoints = props.tooltipMode === 'none' ? [] : props.tooltipMode === 'nearest' ? (current ? [current] : []) : props.tooltipMode === 'intersect' ? (tooltipIntersected.value && current ? [current] : []) : current ? flat.filter((item) => item.index === current.index) : [];
        ...
        if (props.yAxis?.position === 'end') axisNodes.push(h('g', { 'data-part': 'y-axis', transform: `translate(${layout.plotWidth + layout.left * 2} 0)` }, chartTicks(secondaryFullY, props.yAxis.ticks ?? 5).map((tick) => h('text', { x: 8, y: secondaryYMap(tick), 'text-anchor': 'start' }, formatTick(tick, props.yAxis)))));
        ...
        const seriesNodes = [...axisNodes, ...referenceAreaNodes, ...referenceNodes, ...annotationNodes, ...labelNodes, ...prepared.map((item, seriesIndex) => {
          ...
          if (item.type === 'line') return h('g', { 'data-part': 'series', 'data-series': item.id }, [h('path', { d: chartCurvePath(points, item.curve, item.tension), fill: 'none', stroke: color, 'stroke-width': item.lineWidth, 'stroke-dasharray': item.lineDash }), item.pointSymbol && item.points.map((point) => item.pointSymbol === 'square' ? h('rect', { 'data-part': 'point-symbol', x: point.x - 3, y: point.y - 3, width: 6, height: 6, fill }) : item.pointSymbol === 'diamond' ? h('path', { 'data-part': 'point-symbol', d: `M${point.x},${point.y - 4}L${point.x + 4},${point.y}L${point.x},${point.y + 4}L${point.x - 4},${point.y}Z`, fill }) : h('circle', { 'data-part': 'point-symbol', cx: point.x, cy: point.y, r: 3, fill }))]);
          if (item.type === 'area') return h('path', { 'data-part': 'series', 'data-series': item.id, d: item.stack ? stackedAreaPath(item.points.map((point) => ({ x: point.x, y0: point.y0, y1: point.y }))) : areaPath(points, baseline), fill, stroke: color });
          if (item.type === 'bar') return h('g', { 'data-part': 'series', 'data-series': item.id }, item.points.map((point) => { const origin = item.stack ? point.y0 : baseline; const style = chartVisualStyle(point.yValue, props.visualMap); return h('rect', { x: point.x - (bands?.bandwidth ?? 8) / 2, y: Math.min(point.y, origin), width: bands?.bandwidth ?? 8, height: Math.abs(point.y - origin), fill: style.color ?? fill, opacity: style.opacity }); }));
          return h('g', { 'data-part': 'series', 'data-series': item.id }, item.points.map((point) => { const style = chartVisualStyle(point.yValue, props.visualMap); const radius = style.size ?? (item.type === 'bubble' ? point.radius : 3); if (item.pointSymbol === 'square') return h('rect', { x: point.x - radius, y: point.y - radius, width: radius * 2, height: radius * 2, fill: style.color ?? fill, opacity: style.opacity }); if (item.pointSymbol === 'diamond') return h('path', { d: `M${point.x},${point.y - radius}L${point.x + radius},${point.y}L${point.x},${point.y + radius}L${point.x - radius},${point.y}Z`, fill: style.color ?? fill, opacity: style.opacity }); return h('circle', { cx: point.x, cy: point.y, r: radius, fill: style.color ?? fill, opacity: style.opacity }); }));
        })];
        if (useCanvas) {
        ...
        return h('figure', { ...attrs, class: ['simurgh-chart', attrs.class], 'data-slot': 'chart', 'data-motion': props.motion ? 'on' : 'off', 'data-renderer': props.renderMode === 'webgl' ? 'webgl' : useCanvas ? 'canvas' : 'svg', 'aria-hidden': decorative || undefined }, [
          ...
          h('div', { 'data-part': 'viewport', style: { aspectRatio: `${props.width} / ${props.height}` }, onWheel, onMousemove: onMouseMove,
            ...
            onClick: (event: MouseEvent) => { const point = interactionPoint(pointIndexFromEvent(event)); if (point) { tooltipVisible.value = true; props.onPointClick?.(point); if (props.onDrilldown) { props.onDrilldown(point); drilldownStatus.value = (props.locale?.drilldownState ?? defaultChartLocale.drilldownState)(point.seriesId); } } },
            ...
            h('svg', { viewBox: `0 0 ${props.width} ${props.height}`, 'data-part': 'plot', 'aria-hidden': 'true' }, [...(useCanvas ? [] : seriesNodes), h('g', { 'data-part': 'crosshair' }, [h('line', { x1: current.x, x2: current.x, y1: layout.top, y2: layout.top + layout.plotHeight }), h('line', { x1: layout.left, x2: layout.left + layout.plotWidth, y1: current.y, y2: current.y }), h('text', { x: current.x + 6, y: layout.top + 14 }, formatTick(current.xValue, props.xAxis)), h('text', { x: layout.left + 6, y: current.y - 6 }, formatTick(current.yValue, props.yAxis)), h('circle', { cx: current.x, cy: current.y, r: 4 })]), selection.value && h('g', { 'data-part': 'brush' }, [h('rect', { x: selection.value.start[0], y: selection.value.start[1], width: selection.value.end[0] - selection.value.start[0], height: selection.value.end[1] - selection.value.start[1] }), h('rect', { 'data-part': 'brush-handle', x: selection.value.start[0] - 4, y: selection.value.start[1] - 4, width: 8, height: 8 }), h('rect', { 'data-part': 'brush-handle', x: selection.value.end[0] - 4, y: selection.value.start[1] - 4, width: 8, height: 8 }), h('rect', { 'data-part': 'brush-handle', x: selection.value.start[0] - 4, y: selection.value.end[1] - 4, width: 8, height: 8 }), h('rect', { 'data-part': 'brush-handle', x: selection.value.end[0] - 4, y: selection.value.end[1] - 4, width: 8, height: 8 })])]),
            h('button', { type: 'button', 'data-part': 'keyboard-target', 'aria-label': props.locale?.explore ?? defaultChartLocale.explore, onKeydown: (event: KeyboardEvent) => { if (props.interaction && (['+', '=', '-', 'Escape'].includes(event.key) || event.shiftKey && ['ArrowLeft', 'ArrowRight'].includes(event.key))) { onChartKeydown(event); return; } if (event.key === 'Home') focused.value = 0; else if (event.key === 'End') focused.value = flat.length - 1; else if (['ArrowLeft', 'ArrowUp'].includes(event.key)) focused.value = Math.max(0, focused.value - 1); else if (['ArrowRight', 'ArrowDown'].includes(event.key)) focused.value = Math.min(flat.length - 1, focused.value + 1); else return; tooltipVisible.value = true; event.preventDefault(); } }),
            props.interaction && h('button', { type: 'button', 'data-part': 'reset-viewport', onClick: () => { setViewport({}); selection.value = null; props.sync?.set({ selection: null, focused: null }); emit('update:selection', null); props.onSelectedDataChange?.([]); } }, props.locale?.reset ?? defaultChartLocale.reset),
            props.drilldownDepth && props.drilldownDepth > 0 && h('button', { type: 'button', 'data-part': 'drilldown-back', onClick: props.onDrilldownBack }, props.locale?.back ?? defaultChartLocale.back),
            props.streamControls && props.stream && h('button', { type: 'button', 'data-part': 'stream-toggle', 'aria-pressed': streamPaused.value, onClick: () => { if (streamPaused.value) props.stream?.resume(); else props.stream?.pause(); streamPaused.value = !streamPaused.value; } }, streamPaused.value ? props.locale?.resumeStream ?? defaultChartLocale.resumeStream : props.locale?.pauseStream ?? defaultChartLocale.pauseStream),
            props.stream && props.streamAnnouncement && h('div', { 'data-part': 'stream-announcement', 'aria-live': 'polite' }, (props.locale?.dataPoints ?? defaultChartLocale.dataPoints)(props.stream.length, props.streamAutoScroll)),
            (drilldownStatus.value || selection.value || viewport.x || viewport.y) && h('div', { 'data-part': 'interaction-announcement', 'aria-live': 'polite', class: 'simurgh-visually-hidden' }, drilldownStatus.value || (selection.value ? (props.locale?.selectionState ?? defaultChartLocale.selectionState)(new Set(flat.filter((item) => item.x >= selection.value!.start[0] && item.x <= selection.value!.end[0] && item.y >= selection.value!.start[1] && item.y <= selection.value!.end[1]).map((item) => item.datum)).size) : (props.locale?.viewportState ?? defaultChartLocale.viewportState)(viewport.x, viewport.y))),
            tooltipPoints.length && tooltipVisible.value && h('div', { role: 'tooltip', 'data-part': 'tooltip', style: props.tooltipPosition === 'cursor' && tooltipPoint.value ? { position: 'absolute', left: `${tooltipPoint.value[0]}px`, top: `${tooltipPoint.value[1]}px` } : undefined }, props.tooltipContent ? props.tooltipContent(tooltipInteractions) : tooltipPoints.map((item, index) => h('div', { key: `${item.definition.id}:${item.index}` }, props.tooltipFormatter?.(tooltipInteractions[index]!) ?? `${item.definition.label ?? item.definition.id}: ${formatChartValue(item.yValue, props.yAxis?.locale)}`))),
          ]),
          renderLegend(definitions, hiddenSeries),
          table && h('div', { 'data-part': 'data-table' }, [
            h('table', [h('thead', h('tr', [h('th', { scope: 'col' }, props.locale?.category ?? defaultChartLocale.category), ...definitions.map((item) => h('th', { scope: 'col' }, item.label ?? item.id))])), h('tbody', rows.slice(tablePage.value * pageSize, tablePage.value * pageSize + pageSize).map((datum, row) => h('tr', [h('td', String(chartValue(datum, xAccessor, tablePage.value * pageSize + row) ?? '')), ...definitions.map((item) => h('td', String(chartValue(datum, item.y, tablePage.value * pageSize + row) ?? '')))])))]),
            tablePages > 1 && h('nav', { 'aria-label': props.locale?.dataPages ?? defaultChartLocale.dataPages }, [h('button', { type: 'button', disabled: tablePage.value === 0, onClick: () => tablePage.value-- }, props.locale?.previous ?? defaultChartLocale.previous), h('span', `${tablePage.value + 1} / ${tablePages}`), h('button', { type: 'button', disabled: tablePage.value + 1 >= tablePages, onClick: () => tablePage.value++ }, props.locale?.next ?? defaultChartLocale.next)]),
          ]),
        ]);
      };
    },
...
function polar(donut: boolean) {
  return defineComponent({
    ...
    setup(props, { attrs }) { const rowsForChart = useRows(props); const focused = ref(0); const drilldownStatus = ref(''); return () => {
      const rows = rowsForChart();
      ...
      const current = arcs[Math.min(focused.value, arcs.length - 1)];
      const select = (arc: typeof arcs[number], index: number) => { focused.value = index; const slice = { datum: arc.datum, index: arc.index, value: arc.value }; props.onSliceSelect?.(slice); if (props.onDrilldown) { props.onDrilldown(slice); drilldownStatus.value = (props.locale?.drilldownState ?? defaultChartLocale.drilldownState)(String(chartValue(arc.datum, props.x ?? ((_, item) => item), arc.index) ?? arc.index + 1)); } };
      return h('figure', { ...attrs, class: ['simurgh-chart', attrs.class], 'data-slot': 'chart', 'data-state': arcs.length ? undefined : 'empty', 'aria-hidden': decorative || undefined }, arcs.length ? [
        !decorative && h('figcaption', props.accessibility.title),
        h('svg', { viewBox: `${-props.width / 2} ${-props.height / 2} ${props.width} ${props.height}`, 'data-part': 'plot', 'aria-hidden': 'true' }, [arcs.map((arc, index) => h('path', { 'data-part': 'series', d: arc.path, fill: colors[index % colors.length], opacity: index === focused.value ? 1 : 0.7, onMouseenter: () => { focused.value = index; }, onClick: () => select(arc, index) })), labels.length > 0 && h('g', { 'data-part': 'data-labels' }, labels.map((item) => h('text', { 'data-part': 'data-label', x: item.x, y: item.y, 'text-anchor': 'middle' }, item.text))), (props.centerLabel || props.showTotal) && h('g', { 'data-part': 'center-label' }, [props.centerLabel && h('text', { 'text-anchor': 'middle', dy: -4 }, props.centerLabel), props.showTotal && h('text', { 'text-anchor': 'middle', dy: props.centerLabel ? 14 : 4 }, String(arcs.reduce((total, arc) => total + arc.value, 0)))])]),
        h('button', { type: 'button', 'data-part': 'keyboard-target', 'aria-label': props.locale?.explore ?? defaultChartLocale.explore, onKeydown: (event: KeyboardEvent) => { if (event.key === 'Home') focused.value = 0; else if (event.key === 'End') focused.value = arcs.length - 1; else if (['ArrowLeft', 'ArrowUp'].includes(event.key)) focused.value = Math.max(0, focused.value - 1); else if (['ArrowRight', 'ArrowDown'].includes(event.key)) focused.value = Math.min(arcs.length - 1, focused.value + 1); else return; event.preventDefault(); } }),
        props.drilldownDepth && props.drilldownDepth > 0 && h('button', { type: 'button', 'data-part': 'drilldown-back', onClick: props.onDrilldownBack }, props.locale?.back ?? defaultChartLocale.back),
        !decorative && current && h('div', { 'data-part': 'point-announcement', 'aria-live': 'polite', class: 'simurgh-visually-hidden' }, `${String(chartValue(current.datum, props.x ?? ((_, item) => item), current.index) ?? current.index + 1)}: ${current.value}`),
        !decorative && drilldownStatus.value && h('div', { 'data-part': 'interaction-announcement', 'aria-live': 'polite', class: 'simurgh-visually-hidden' }, drilldownStatus.value),
        !decorative && h('p', { 'data-part': 'description' }, `${props.accessibility.description} ${chartSummary(arcs.map((arc) => arc.value), 'Slices')}`),
      ] : props.emptyContent);
    }; },
  });
}
