      ...
      const tooltipPoints = props.tooltipMode === 'none' ? [] : props.tooltipMode === 'nearest' ? (current ? [current] : []) : props.tooltipMode === 'intersect' ? (tooltipIntersected.value && current ? [current] : []) : current ? flat.filter((item) => item.index === current.index) : [];
        ...
        return h('figure', { ...attrs, class: ['simurgh-chart', attrs.class], 'data-slot': 'chart', 'data-motion': props.motion ? 'on' : 'off', 'data-renderer': useCanvas ? 'canvas-fallback' : 'svg', 'aria-hidden': decorative || undefined }, [
          ...
          h('div', { 'data-part': 'viewport', style: { aspectRatio: `${props.width} / ${props.height}` }, onWheel, onMousemove: onMouseMove,
            ...
            props.stream && props.streamAnnouncement && h('div', { 'data-part': 'stream-announcement', 'aria-live': 'polite' }, `${props.stream.length} data points${props.streamAutoScroll ? ', following latest data' : ''}`),
