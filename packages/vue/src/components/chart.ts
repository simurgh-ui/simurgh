...
function polar(donut: boolean) {
  return defineComponent({
    ...
    setup(props, { attrs }) { const rowsForChart = useRows(props); return () => {
      ...
      return h('figure', { ...attrs, class: ['simurgh-chart', attrs.class], 'data-slot': 'chart', 'data-state': arcs.length ? undefined : 'empty', 'aria-hidden': decorative || undefined }, arcs.length ? [!decorative && h('figcaption', props.accessibility.title), h('svg', { viewBox: `${-props.width / 2} ${-props.height / 2} ${props.width} ${props.height}`, 'data-part': 'plot', 'aria-hidden': 'true' }, [arcs.map((arc, index) => h('path', { 'data-part': 'series', d: arc.path, fill: colors[index % colors.length], onMouseenter: () => { const slice = { datum: arc.datum, index: arc.index, value: arc.value }; props.onSliceSelect?.(slice); props.onDrilldown?.(slice); }, onClick: () => { const slice = { datum: arc.datum, index: arc.index, value: arc.value }; props.onSliceSelect?.(slice); props.onDrilldown?.(slice); } })), labels.length > 0 && h('g', { 'data-part': 'data-labels' }, labels.map((item) => h('text', { 'data-part': 'data-label', x: item.x, y: item.y, 'text-anchor': 'middle' }, item.text))), (props.centerLabel || props.showTotal) && h('g', { 'data-part': 'center-label' }, [props.centerLabel && h('text', { 'text-anchor': 'middle', dy: -4 }, props.centerLabel), props.showTotal && h('text', { 'text-anchor': 'middle', dy: props.centerLabel ? 14 : 4 }, String(arcs.reduce((total, arc) => total + arc.value, 0)))])]), props.drilldownDepth && props.drilldownDepth > 0 && h('button', { type: 'button', 'data-part': 'drilldown-back', onClick: props.onDrilldownBack }, props.locale?.back ?? defaultChartLocale.back), !decorative && h('p', { 'data-part': 'description' }, `${props.accessibility.description} ${chartSummary(arcs.map((arc) => arc.value), 'Slices')}`)] : props.emptyContent);
    }; },
  });
}
