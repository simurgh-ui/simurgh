...
describe('Vue shared chart contract', () => {
  it('executes the capability and accessibility contract', async () => {
    await runSharedChartContract(() => {
      ...
      const view = render(LineChart, { props: { data: [{ x: 0, y: 2 }, { x: 10, y: 8 }], x: 'x', y: 'y', interaction: { zoom: 'x' }, onPointClick: pointClick, onDrilldown: () => {}, 'onUpdate:viewport': viewportChange, accessibility: { title: 'Contract chart', description: 'Two values.', table: { pageSize: 1 } }, locale: { explore: 'Explorer', reset: 'Réinitialiser', category: 'Catégorie', dataPages: 'Pages de données' } } });
