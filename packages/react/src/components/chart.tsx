...
function SeriesMarks<T>({ item, index, baseline, bandwidth, orientation, visualMap }: { item: PreparedSeries<T>; index: number; baseline: number; bandwidth: number; orientation: string; visualMap: ChartVisualMap | undefined }) {
  ...
  if (item.type === 'line') return <g data-part="series" data-series={item.id}><path d={chartCurvePath(points, item.curve)} fill="none" stroke={color} strokeWidth={item.lineWidth} strokeDasharray={item.lineDash} />{item.pointSymbol && item.points.map((point) => <circle key={point.index} data-part="point-symbol" cx={point.x} cy={point.y} r="3" fill={color} />)}</g>;
  if (item.type === 'area') return <path data-part="series" data-series={item.id} d={item.stack ? stackedAreaPath(item.points.map((point) => ({ x: point.x, y0: point.y0, y1: point.y }))) : areaPath(points, baseline)} fill={color} stroke={color} strokeWidth={item.lineWidth} strokeDasharray={item.lineDash} />;
  ...
  return <g data-part="series" data-series={item.id}>{item.points.map((point) => { const style = chartVisualStyle(point.yValue, visualMap); const radius = style.size ?? (item.type === 'bubble' ? point.radius : 3); if (item.pointSymbol === 'square') return <rect key={point.index} x={point.x - radius} y={point.y - radius} width={radius * 2} height={radius * 2} fill={style.color ?? color} opacity={style.opacity} />; if (item.pointSymbol === 'diamond') return <path key={point.index} d={`M${point.x},${point.y - radius}L${point.x + radius},${point.y}L${point.x},${point.y + radius}L${point.x - radius},${point.y}Z`} fill={style.color ?? color} opacity={style.opacity} />; return <circle key={point.index} cx={point.x} cy={point.y} r={radius} fill={style.color ?? color} opacity={style.opacity} />; })}</g>;
}
