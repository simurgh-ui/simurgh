...
Accessible, tree-shakeable React components from Simurgh UI. The package supports React 18 and
newer and exposes every component through a granular subpath.
...
React and React DOM are peer dependencies and must be installed by the application.
...
- `@simurgh-ui/icons` provides per-icon React entry points.
- `@simurgh-ui/motion/react` provides optional animation primitives.
