...
function validateFiles(files: File[]) {
  const accepted = files.filter((file) =>
    file.type === 'application/pdf' && file.size <= MAX_BYTES);
  setErrors(accepted.length === files.length ? [] : ['Use PDF files up to 5 MB.']);
  setFiles(accepted);
}
<FileUpload label="Supporting documents" accept="application/pdf" multiple
  name="documents" onFilesChange={validateFiles} />
```
