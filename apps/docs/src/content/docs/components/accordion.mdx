...
### Real-world example: dynamic FAQ items

Use stable values so expanded state follows an item across filtering or reordering.
...
Vue renders the same collection with `v-for="item in faqItems"` and `:key="item.id"`. Angular uses
