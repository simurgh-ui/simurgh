...
| Disabled | Not supported by this component API; handle this state in surrounding application UI. | Not supported by this component API; handle this state in surrounding application UI. | Supported with `[disabled]="true"` on the documented control or interactive part; its interaction is blocked. Disabled form controls are omitted from submission. |
| Error | Use the dedicated `FieldError` region. | Use the dedicated `FieldError` region. | Use the dedicated `FieldErrorComponent` region. |
...
| Stable DOM parts | `[data-slot="field"]`, `[data-slot="field-description"]`, `[data-slot="field-error"]`, `[data-slot="field-legend"]` |
| Stable data attributes | `[data-slot]` |
| ARIA/state selectors used by the recipe | No ARIA state selector is used by this recipe. |
| CSS custom properties consumed | `--simurgh-danger`, `--simurgh-muted-foreground` |
