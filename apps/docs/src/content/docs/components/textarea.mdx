...
<form onSubmit={(event) => {
  ...
  <Textarea name="notes" value={value} onChange={(event) => setValue(event.currentTarget.value)}
    required disabled={isDisabled} invalid={Boolean(errors.notes)} />
...
<form #form (submit)="submit(form); $event.preventDefault()">
  <simurgh-textarea [(value)]="value" name="notes" required [disabled]="isDisabled" [invalid]="!!errors.notes"></simurgh-textarea>
