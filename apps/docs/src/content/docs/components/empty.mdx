...
| --- | --- | --- | --- |
| React | Render `Empty` as the ancestor. Nest the listed parts inside that root so they can read its shared state/context. | `EmptyHeader`, `EmptyMedia`, `EmptyTitle`, `EmptyDescription`, `EmptyContent` are conditional parts: include only those needed by the documented anatomy. A trigger/control and its matching content/item are required when that interaction is used. | None. |
| Vue | Render `Empty` as the ancestor. Nest the listed parts inside that root so they can read its shared state/context. | `EmptyHeader`, `EmptyMedia`, `EmptyTitle`, `EmptyDescription`, `EmptyContent` are conditional parts: include only those needed by the documented anatomy. A trigger/control and its matching content/item are required when that interaction is used. | None. |
| Angular | Create `EmptyComponent` first. Attach listed directives to elements inside its projected content; nested component parts must remain inside it. | `EmptyHeaderComponent`, `EmptyMediaComponent`, `EmptyTitleComponent`, `EmptyDescriptionComponent`, `EmptyContentComponent` are conditional parts: include only those needed by the documented anatomy. A trigger/control and its matching content/item are required when that interaction is used. | None. |
...
| Empty | Dedicated empty-state primitive; compose its title, description, media, and actions. | Dedicated empty-state primitive; compose its title, description, media, and actions. | Dedicated empty-state primitive; compose its title, description, media, and actions. |
...
| Stable DOM parts | `[data-slot="empty"]`, `[data-slot="empty-content"]`, `[data-slot="empty-description"]`, `[data-slot="empty-header"]`, `[data-slot="empty-media"]`, `[data-slot="empty-title"]` |
| Stable data attributes | `[data-slot]` |
| ARIA/state selectors used by the recipe | No ARIA state selector is used by this recipe. |
| CSS custom properties consumed | `--simurgh-border`, `--simurgh-muted`, `--simurgh-muted-foreground`, `--simurgh-radius` |
