...
Popover uses Simurgh's lightweight positioning layer for offset, viewport flip, boundary shift, and automatic repositioning. Outside interaction and Escape dismiss it without trapping focus.
...
| --- | --- |
| Focus entry and exit (Tab / Shift+Tab) | Tab reaches the trigger and then focusable popover content in DOM order. |
| Navigation and activation | Enter or Space toggles the trigger and activates focused content controls. |
| Escape | Closes the popover when focus is inside its content. |
