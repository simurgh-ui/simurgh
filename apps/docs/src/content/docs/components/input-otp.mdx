...
<form #form (submit)="submit(form); $event.preventDefault()">
  ...
  ></simurgh-input-otp>
  <p *ngIf="errors.verificationCode" role="alert">
...
| Stable DOM parts | `[data-slot="input-otp"]` |
| Stable data attributes | `[data-slot]` |
| ARIA/state selectors used by the recipe | `[aria-invalid]` |
| CSS custom properties consumed | `--simurgh-border`, `--simurgh-danger`, `--simurgh-foreground`, `--simurgh-otp-length`, `--simurgh-radius`, `--simurgh-ring`, `--simurgh-surface` |
