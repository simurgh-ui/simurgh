...
async function submitProfile(event: FormEvent<HTMLFormElement>) {
  ...
  try { await saveProfile(new FormData(event.currentTarget)); setOpen(false); }
  catch { setMessage('Could not save the profile. Try again.'); }
  finally { setSaving(false); }
}
<Dialog open={open} onOpenChange={setOpen}>…<form onSubmit={submitProfile}>
