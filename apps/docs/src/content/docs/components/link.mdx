...
```ts
import { Link } from '@simurgh-ui/vue/link';
import '@simurgh-ui/styles/link.css';
```
...
Component selector: `simurgh-link`. The template's first native element is `a`.
...
| --- | --- | --- | --- |
| React | `Link` is standalone and required when using this component. | Content is optional unless the API requires a label or value. | None. |
| Vue | `Link` is standalone and required when using this component. | Content is optional unless the API requires a label or value. | None. |
| Angular | `LinkComponent` is standalone and required when using this component. | Content is optional unless the API requires a label or value. | None. |
...
| Stable DOM parts | `[data-slot="link"]` |
| Stable data attributes | `[data-external]`, `[data-slot]` |
| ARIA/state selectors used by the recipe | `[aria-disabled]` |
| CSS custom properties consumed | `--simurgh-accent`, `--simurgh-muted-foreground`, `--simurgh-primary`, `--simurgh-radius`, `--simurgh-ring` |
