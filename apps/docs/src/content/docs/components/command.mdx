...
useEffect(() => {
  ...
  fetch(`/api/commands?q=${encodeURIComponent(query)}`, { signal: request.signal })
    .then((response) => response.json()).then(setServerCommands)
    .finally(() => setLoading(false));
  return () => request.abort();
}, [query]);
<Command options={serverCommands} onValueChange={runCommand}
