...
import {
  ...
  DescriptionListTerm,
  DescriptionListDetails,
} from '@simurgh-ui/vue/description-list';
import '@simurgh-ui/styles/description-list.css';
```
...
} from '@simurgh-ui/angular/description-list';
import '@simurgh-ui/styles/description-list.css';
```
...
- React exports: `DescriptionList`, `DescriptionListGroup`, `DescriptionListTerm`, `DescriptionListDetails`
- Vue exports: `DescriptionList`, `DescriptionListGroup`, `DescriptionListTerm`, `DescriptionListDetails`
- Angular exports: `DescriptionListDirective`, `DescriptionListGroupDirective`, `DescriptionListTermDirective`, `DescriptionListDetailsDirective`
...
Inherited attributes: `HTMLAttributes<HTMLDListElement> & RefAttributes<HTMLDListElement>`.

| Prop | Type | Default / requirement |
| --- | --- | --- |
| `children` | `ReactNode` | `undefined` |
| `ref` | `Ref<HTMLDListElement> \| undefined` | `undefined` |
...
Attribute directive selector: `dl[simurghDescriptionList]`; behavior applies to its host element.
...
Attribute directive selector: `div[simurghDescriptionListGroup]`; behavior applies to its host element.
...
Attribute directive selector: `dt[simurghDescriptionListTerm]`; behavior applies to its host element.
...
Attribute directive selector: `dd[simurghDescriptionListDetails]`; behavior applies to its host element.
...
| Stable DOM parts | `[data-slot="description-list"]`, `[data-slot="description-list-details"]`, `[data-slot="description-list-group"]`, `[data-slot="description-list-term"]` |
| Stable data attributes | `[data-slot]` |
| ARIA/state selectors used by the recipe | No ARIA state selector is used by this recipe. |
| CSS custom properties consumed | `--simurgh-border`, `--simurgh-foreground`, `--simurgh-muted-foreground` |
...
| --- | --- | --- | --- |
| React | Render `DescriptionList` as the ancestor. Nest the listed parts inside that root so they can read its shared state/context. | `DescriptionListGroup`, `DescriptionListTerm`, `DescriptionListDetails` are conditional parts: include only those needed by the documented anatomy. A trigger/control and its matching content/item are required when that interaction is used. | None. |
| Vue | Render `DescriptionList` as the ancestor. Nest the listed parts inside that root so they can read its shared state/context. | `DescriptionListGroup`, `DescriptionListTerm`, `DescriptionListDetails` are conditional parts: include only those needed by the documented anatomy. A trigger/control and its matching content/item are required when that interaction is used. | None. |
| Angular | Create `DescriptionListDirective` first. Attach listed directives to elements inside its projected content; nested component parts must remain inside it. | `DescriptionListGroupDirective`, `DescriptionListTermDirective`, `DescriptionListDetailsDirective` are conditional parts: include only those needed by the documented anatomy. A trigger/control and its matching content/item are required when that interaction is used. | None. |
...
- Inspect the registry [manifest](https://github.com/simurgh-ui/simurgh/blob/main/packages/registry/registry.json) and framework
  ...
  [Vue](https://github.com/simurgh-ui/simurgh/blob/main/packages/vue/src/components/description-list.ts), or
  [Angular](https://github.com/simurgh-ui/simurgh/blob/main/packages/angular/src/components/description-list.ts).
