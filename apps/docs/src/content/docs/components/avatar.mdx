...
| `imageProps` | `ImgHTMLAttributes<HTMLImageElement>` | `undefined` |
...
Component selector: `simurgh-avatar`. The template's first native element is `img`. Angular host bindings: `{ '[attr.data-state]': "loaded ? 'loaded' : 'fallback'" }`.
...
| --- | --- | --- | --- |
| React | `Avatar` is standalone and required when using this component. | Content is optional unless the API requires a label or value. | None. |
| Vue | `Avatar` is standalone and required when using this component. | Content is optional unless the API requires a label or value. | None. |
| Angular | `AvatarComponent` is standalone and required when using this component. | Content is optional unless the API requires a label or value. | None. |
