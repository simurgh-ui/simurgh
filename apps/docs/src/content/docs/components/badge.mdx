...
Component selector: `simurgh-badge`. The template's first native element is `ng-content`. Angular host bindings: `{ '[attr.data-tone]': 'tone', '[attr.role]': "status ? 'status' : null", '[attr.aria-live]': "status ? 'polite' : null", }`.
...
| --- | --- | --- | --- |
| React | `Badge` is standalone and required when using this component. | Content is optional unless the API requires a label or value. | `BadgeTone`. |
| Vue | `Badge` is standalone and required when using this component. | Content is optional unless the API requires a label or value. | `BadgeTone`. |
| Angular | `BadgeComponent` is standalone and required when using this component. | Content is optional unless the API requires a label or value. | `BadgeTone`. |
...
| Stable data attributes | `[data-tone]` |
