...
Component selector: `simurgh-input-group`. The template's first native element is `ng-content`. Angular host bindings: `{ role: 'group', '[attr.data-slot]': "'input-group'" }`.
...
Component selector: `simurgh-input-group-addon`. The template's first native element is `ng-content`. Angular host bindings: `{ '[attr.aria-hidden]': 'decorative \|\| null', '[attr.data-align]': 'align', '[attr.data-slot]': "'input-group-addon'", }`.
...
Component selector: `simurgh-input-group-text`. The template's first native element is `ng-content`. Angular host bindings: `{ '[attr.data-slot]': "'input-group-text'" }`.
...
| --- | --- | --- | --- |
| React | Render `InputGroup` as the ancestor. Nest the listed parts inside that root so they can read its shared state/context. | `InputGroupAddon`, `InputGroupText` are conditional parts: include only those needed by the documented anatomy. A trigger/control and its matching content/item are required when that interaction is used. | None. |
| Vue | Render `InputGroup` as the ancestor. Nest the listed parts inside that root so they can read its shared state/context. | `InputGroupAddon`, `InputGroupText` are conditional parts: include only those needed by the documented anatomy. A trigger/control and its matching content/item are required when that interaction is used. | None. |
| Angular | Create `InputGroupComponent` first. Attach listed directives to elements inside its projected content; nested component parts must remain inside it. | `InputGroupAddonComponent`, `InputGroupTextComponent` are conditional parts: include only those needed by the documented anatomy. A trigger/control and its matching content/item are required when that interaction is used. | None. |
...
| Stable DOM parts | `[data-slot="input-group"]` |
| Stable data attributes | `[data-align]`, `[data-slot]` |
